...
try:
    ...
    latency_csv = "./logs/latency_data_no_qos.csv"
    ...
    print(f"[+] Saved latency data to {latency_csv}")
    ...
    summary_csv = "./logs/summary_stats_no_qos.csv"
    ...
    plt.title("H.264 Frame Latency (No QoS)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("./plots/latency_plot_h264_no_qos.png")
    print("[+] Saved latency plot: ./plots/latency_plot_h264_no_qos.png")

    # Frame loss plot
    plt.figure()
    plt.bar(["Received", "Lost"], [len(frame_ids_received), len(frame_ids_lost)], color=["green", "red"])
    plt.title("H.264 Frame Loss (No QoS)")
    plt.tight_layout()
    plt.savefig("./plots/frame_loss_plot_h264_no_qos.png")
    print("[+] Saved frame loss plot: ./plots/frame_loss_plot_h264_no_qos.png")
