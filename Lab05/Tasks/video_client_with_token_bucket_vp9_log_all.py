...
try:
    while True:
        ...
        # Timestamps: capture, encode start, encode end
        ...
        # Decode VP9 to raw video using ffmpeg
    ...
    # Save latency data
    latency_csv = os.path.join(LOG_DIR, "latency_data_token_bucket_log_all_vp9.csv")
    ...
    # Save CPU usage data
    cpu_csv = os.path.join(LOG_DIR, "cpu_usage_log_all_vp9.csv")
    ...
    # Save bandwidth data
    bw_csv = os.path.join(LOG_DIR, "bandwidth_log_all_vp9.csv")
    ...
    summary_csv = os.path.join(LOG_DIR, "summary_stats_token_bucket_log_all_vp9.csv")
    summary_df.to_csv(summary_csv, index=False)
    print(f"[+] Saved summary stats: {summary_csv}")

    # Latency plot
    plt.figure(figsize=(10, 4))
    plt.plot(frame_ids_received, latencies, marker='o', linestyle='-', color='orange')
    plt.xlabel("Frame ID")
    plt.ylabel("Latency (s)")
    plt.title("VP9 Frame Latency (Token Bucket)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, "latency_plot_vp9_token_bucket_log_all.png"))
    print(f"[+] Saved latency plot.")

    # Frame loss plot
    plt.figure()
    plt.bar(["Received", "Lost"], [len(frame_ids_received), len(frame_ids_lost)], color=["green", "red"])
    plt.title("VP9 Frames Received vs Lost")
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, "frame_loss_plot_vp9_token_bucket_log_all.png"))
