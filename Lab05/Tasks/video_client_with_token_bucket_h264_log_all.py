...
try:
    while True:
        ...
        while len(data) < timestamp_size * 3:
            ...
            if not chunk:
                raise ConnectionError("Disconnected.")
            data += chunk
        timestamps = struct.unpack("ddd", data[:timestamp_size * 3])
    ...
    # Save latency data
    latency_csv = os.path.join(LOG_DIR, "latency_data_token_bucket_log_all.csv")
    ...
    # Save CPU usage data
    cpu_csv = os.path.join(LOG_DIR, "cpu_usage_log_all.csv")
    ...
    # Save bandwidth data
    bw_csv = os.path.join(LOG_DIR, "bandwidth_log_all.csv")
    ...
    summary_csv = os.path.join(LOG_DIR, "summary_stats_token_bucket_log_all.csv")
    ...
    plt.title("H.264 Frame Latency (Token Bucket)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, "latency_plot_h264_token_bucket_log_all.png"))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, "frame_loss_plot_h264_token_bucket_log_all.png"))
